...
const NewUserStep: React.FC<NewUserStepProps> = ({ data }) => {
  ...
  const {
    ...
    defaultValues: {
      nombre: registration?.newUser?.nombre || "",
      email: registration?.newUser?.email || "",
      password: registration?.newUser?.password || "",
      confirmPassword: registration?.newUser?.password || "",
      celular: registration?.newUser?.celular || "",
      countryCode: registration?.newUser?.countryCode || "",
      type: registration?.newUser?.type || "whatsapp",
      status: registration?.newUser?.status || "pending",
      code: 0,
    },
  });
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Label htmlFor="sms" className="text-xs text-neutral-400">
        Número de celular
      </Label>
      <div>
        <PhoneInput
          inputClass={`!py-4 !w-full !pl-20 !h-11 !border-input !text-xs ${
            errors.celular && "!border-red-500"
          }`}
          disabled={isLoading}
          buttonClass={`!w-16 !p-2 !py-4  !h-11 !border-input ${
            errors.celular && "!border-red-500"
          }`}
          country={"mx"}
        ...
        />
        <p className="my-2 text-[11px] text-red-500">
          {errors.celular?.message}
        </p>
      </div>
      <div className="w-full flex flex-col gap-4 my-2">
        <p className="text-xs text-neutral-400">
          Enviar código de confirmación por:{" "}
        </p>
        <RadioGroup
        ...
        >
          <div className="flex items-center space-x-2 mb-1">
            ...
            <Label htmlFor="whatsapp" className="text-xs w-full">
              Mensaje de Whatsapp
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="sms" />
            <Label htmlFor="sms" className="text-xs w-full">
