...
export async function registerOtp(props: RegisterOtpProps) {
  const result = await axios
    .post(`/api/register/verify/send`, props)
    .then((response) => {
      if (response.data.status == 1) {
        ...
        const responseData: ApiResponse = {
          status: 2,
          statusMessage: "No se pudo entregar el mensaje",
...
export async function registerOtpResend(props: RegisterOtpProps) {
  ...
  const result = await axios
    .post(`/api/register/verify/resend`, props)
    .then((response) => {
      if (response.data.status == 1) {
        const responseData: ApiResponse = {
          status: 1,
...
export async function checkOtp(props: RegisterCheckOtpProps) {
  ...
  const result = await axios
    .post(`/api/register/verify/check`, props)
    .then((response) => {
      if (response.data.status == 1) {
        const responseData: ApiResponse = {
          status: 1,
