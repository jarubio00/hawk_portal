...
const EnvioInfo: React.FC<EnvioInfoProps> = ({
 data
}) => {
  ...
  <div className='mx-4 mt-4 flex flex-col gap-2'>
    <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="recoleccion-1">
            <div className="flex flex-col grow-0 text-left text-xs">
                ...
                <p className="text-xs text-neutral-500 truncate ">{namedDateString(data.recoleccion.fecha)} - {bloqueToString(data.recoleccion.bloque)}</p>
                <p className="text-[xs] text-neutral-500 truncate">{data.recoleccion.colonia}, {data.recoleccion?.municipio?.municipio}</p>
                <AccordionTrigger className="py-0 ">
                    <p className="text-[11px] text-blue-500">Ver más...</p>
                </AccordionTrigger>
                <AccordionContent>
                    <p className="text-xs text-neutral-700 truncate ">{data.recoleccion.contactoNombre} ({data.recoleccion.contactoTel})</p>
                    <p className="text-xs text-neutral-500 truncate">{data.recoleccion.calle}, {data.recoleccion?.numero} {data.recoleccion?.numeroInt}</p>
                    <p className="text-xs text-neutral-500 truncate">{data.recoleccion.colonia}, {data.recoleccion?.municipio?.municipio}</p>
                    <p className="text-xs text-neutral-500 truncate">{data.recoleccion.cpId}</p>
                    <p className="text-xs text-neutral-700 font-semibold truncate">{data.recoleccion.empresa}</p>
                    <p className="text-xs text-neutral-500 ">{data.recoleccion.referencias}</p>
                </AccordionContent>
            </div>
        </AccordionItem>
    </Accordion>
    <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="entrega-1">
            ...
            <hr className="my-1"></hr>
                <p className="text-[11px] text-neutral-500 truncate ">{namedDateString(data.fechaEntrega)} - {bloqueToString(data.bloqueEntrega)}</p>
                <p className="text-[11px] text-neutral-500 truncate">{data.entregaColonia}, {data.municipio?.municipio}</p>
                <AccordionTrigger className="py-0 ">
                    <p className="text-[11px] text-blue-500">Ver más...</p>
                </AccordionTrigger>
                <AccordionContent>
    ...
    <div className='flex flex-row gap-1 items-center my-2'>
        ...
        <div className="flex flex-col">
            <p className="text-xs  text-black uppercase">{data.tipoPaquete.tipo}</p>
    ...
    <div className='flex flex-row gap-1 items-center mt-2'>
        ...
        <Badge variant='secondary' className={`${(data.formaPagoId == 2 || data.estatusPagoId == 2 || data.estatusPagoId == 3) &&  'bg-green-500'} py-0`}>
    ...
    {data.cobroDestino && <><p className="text-sm font-semibold mt-2">Cobro a destinatario</p>
    <hr className="my-0"></hr>
    <div className='flex flex-row gap-1 items-center mt-2'>
        <p className="text-sm text-black font-bold">${data.precioVenta}</p>
