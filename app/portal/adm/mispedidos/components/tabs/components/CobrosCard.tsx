      ...
      <div className="flex flex-row items-center justify-between px-2 py-1">
        <div className="flex flex-col">
          <p className="font-semibold text-foreground text-xs ">
            {data.pedido.entregaContactoNombre}
          </p>
          <p className="text-neutral-400 text-xs ">
            {data.pedido?.entregaColonia}, {data.pedido?.municipio?.municipio}
          </p>
        </div>
        <div className="flex ">
      ...
      <div className="px-2 py-1 flex flex-row justify-between items-center">
        <div className="flex flex-col">
          <p className="text-sm text-black font-bold">
            ${currencyFormat(data.cantidad)}
          </p>
          <p className="text-[11px] text-neutral-400 leading-none">
            Comisión: ${(data.cantidad * 0.045).toFixed(2)}
          </p>
        </div>
          ...
          >
            <div className="flex flex-row items-center">
              <HiCheckCircle
                size={14}
                className={`${
                  data.estatusCobroId == 2 || data.estatusCobroId == 3
                    ? "text-green-500"
                    : "text-neutral-300"
                }`}
              />
