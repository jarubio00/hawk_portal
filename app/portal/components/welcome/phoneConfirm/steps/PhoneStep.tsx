...
const PhoneStep: React.FC<PhoneStepProps> = ({ currentUser }) => {
  ...
  const onSubmit: SubmitHandler<PhoneConfirmFormType> = async (
    data: PhoneConfirmFormType
  ) => {
    //console.log(data);
    if (
      data.celular === phoneConfirm?.celular &&
      data.type === phoneConfirm?.type
    ) {
      updateActiveStep(1);
    } else {
      setIsLoading(true);
