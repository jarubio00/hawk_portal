...
const PasswordChange: React.FC<PasswordChangeProps> = ({
    ...
    const validationSchema = Yup.object().shape({
      ...
      const onSubmit: SubmitHandler<PasswordChangeFormType> = async (data: PasswordChangeFormType) => {
        setIsLoading(true);
        toggleCloseButton();
  ...
  <div className='my-0'>
     {!changed ? <div>
        <div className="flex flex-col text-xs text-center text-neutral-400 mb-2 md:mb-4">
          <span className="font-semibold text-black text-sm">Cambiar contraseña</span>
        </div>
        ...
        <Button variant={'ghost'} className="px-2 font-bold text-center" onClick={onForgot}>Olvidé mi contraseña anterior</Button>
