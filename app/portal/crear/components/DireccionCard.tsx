...
const DireccionCard: React.FC<DireccionCardProps> = ({
  ...
  return (
    <div
      className={`mx-0 my-0 p-4 px-2  border-neutral-200 border rounded-lg shadow-md `}
    >
      <div className="flex flex-col ">
        <div className="flex flex-row justify-between items-center">
          <span className="text-sm font-bold">{data?.cpId}</span>
        ...
        {data.empresa && (
          <span className=" flex-wrap text-blue-500 text-xs ">
            {data.empresa}
          </span>
        )}
        {data.referencias && (
        ...
        <span className="text-xs font-semibold mt-4 mb-2">
          Datos de contacto
        </span>
        {!append && (
          <span className="text-xs text-neutral-500 font-base  mb-2">
            Puedes cambiarlos para esta recolección
          </span>
        )}
