...
const MuiDatePicker: React.FC<MuiDatePickerProps> = ({
  ...
  function ButtonField(props: ButtonFieldProps) {
    ...
    return (
      <button
        className="p-2 border  disabled:border-neutral-300 disabled:border shadow-md rounded-md text-black disabled:text-neutral-300 disabled:italic"
        id={id}
        disabled={disabled}
        ref={ref}
  ...
  const disableWeekends = (date: any) => {
    ...
    if (date.getDay() === 0) {
      ...
      if (
        isDate(datetime) &&
        format(datetime, `yyyy-MM-dd`) == date.toISOString().slice(0, 10)
      ) {
