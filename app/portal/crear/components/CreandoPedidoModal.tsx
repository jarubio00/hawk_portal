    ...
    if (pedido) {
      ...
      if (pedidoResult.status == 1) {
        if (pedido.metodoPago?.formaPagoId == 2) {
          if (
            pedido?.metodoPago?.comprobanteImageFile &&
            pedidoResult?.response?.data.pedidoId
          ) {
  ...
  const bodyContent = (
    <div className="flex flex-col h-full gap-2 justify-center items-center">
      <div className="mt-4 mx-auto">
        {isLoading && !pedido?.append?.enabled && (
          <p className="text-lg font-bold">Generando envío...</p>
        )}
        {isLoading && pedido?.append?.enabled && (
      ...
      <div className="w-40 h-28 flex items-center justify-center mx-auto">
        {isLoading ? (
          <PulseLoader size={30} color="#FF6B00" className="ml-0 mt-0" />
        ) : (
          ...
          {guiaBox && (
            <div className="flex flex-col items-center justify-center">
              <p className="text-xs text-center">
                Tu envío fue generado exitosamente, gracias por tu preferencia.
              </p>
              <p className="text-md font-bold text-center mb-1">
                Guía: {pedidoCreado?.pedidoId}
              </p>
  ...
  const footerContent = (
    <div className="flex flex-col items-center justify-center gap-4 mt-1 ">
      ...
      <div className="m-0 w-36 h-10 mx-auto">
        {!isLoading && (
          <Button label="Ir a mi panel" onClick={pedidoExitoso} full />
        )}
      </div>
    </div>
  );
