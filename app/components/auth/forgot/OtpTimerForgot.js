...
function OtpTimer({ expiryTimestamp, onResendCode, email}) {
  ...
  return (
    ...
    <DropdownMenuContent className="m-0 w-64">
        <DropdownMenuLabel className='text-xs text-neutral-400'>{email}</DropdownMenuLabel>
        <DropdownMenuLabel>Reenviar código por:</DropdownMenuLabel>
        <DropdownMenuSeparator />
        <DropdownMenuGroup>
          <DropdownMenuItem className="cursor-pointer text-xs hover:bg-neutral-300 hover:underline" onClick={() => handleOnClick('email')}>
            Email
          </DropdownMenuItem>
